// This script handles curved-text class letter
// positioning on the DOM.
function updateCurvedText($curvedText, radius) {
    ...
    $letters.each(function(idx,el){
      $(el).css({
          transform:`translate(${w/2 + window.innerWidth/4}px,0px) rotate(${idx * angle - text.length*angle/2}deg)`
      })
    });
  }
